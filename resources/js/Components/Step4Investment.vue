...
// NL currency formatter
const fmt = (v) =>
  new Intl.NumberFormat('nl-NL', { style: 'currency', currency: 'EUR' }).format(v || 0)

// Accepteer zowel punt als komma als decimaalteken
  ...
  <div class="space-y-5">
    ...
    <div>
      <div class="grid grid-cols-[auto_1fr_auto_auto] gap-2 mb-2 px-1">
        ...
        <span class="text-xs font-medium text-gray-500">Omschrijving</span>
        <span class="text-xs font-medium text-gray-500 w-32 text-right">Bedrag (€)</span>
        <span class="w-7" />
      </div>
      ...
      >
        <template #item="{ element: row, index }">
          <div class="grid grid-cols-[auto_1fr_auto_auto] gap-2 items-center bg-white rounded-lg border border-gray-200 px-3 py-2">
            <!-- Drag handle -->
            <span class="drag-handle cursor-grab text-gray-300 hover:text-gray-500 select-none text-lg leading-none">⠿</span>
    ...
    <!-- BTW toggle -->
    <div>
      <label class="text-sm font-medium text-gray-700 block mb-2">BTW</label>
    ...
    <!-- Totalen -->
    <div class="rounded-lg bg-gray-50 border border-gray-200 divide-y divide-gray-200">
      <div class="flex justify-between px-4 py-2.5 text-sm text-gray-600">
        <span>Subtotaal</span>
        <span class="font-mono">{{ fmt(subtotaal) }}</span>
      </div>

      <div v-if="btw === '21%'" class="flex justify-between px-4 py-2.5 text-sm text-gray-600">
        <span>BTW (21%)</span>
        <span class="font-mono">{{ fmt(btwBedrag) }}</span>
      </div>
      <div v-else class="flex justify-between px-4 py-2.5 text-sm text-gray-400 italic">
        <span>BTW</span>
    ...
    <p v-if="!canProceed" class="text-xs text-amber-600">
